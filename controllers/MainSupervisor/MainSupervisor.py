...
class Erebus(Supervisor):
    ...
    def __init__(self):
        ...
        self._game_state: GameState = GameState.MATCH_RUNNING
        self._last_frame: Optional[bool] = False
        self._first_frame: bool = True
        self._robot_initialised: bool = False
        self._demo_route_enabled: bool = self.config.fixed_route_enabled
        self._demo_route_distance: float = 0.0
        self._demo_route_speed: float = 0.075
        # Centerline waypoints for a deterministic recording route.  The
        # supervisor drives the pose directly, so every segment must stay in
        # open corridor space rather than relying on collision response.
        self._demo_route_points: list[tuple[float, float]] = [
            (-0.48, -0.48),
            (-0.48, 0.12),
            (-0.36, 0.12),
            (-0.36, 0.02),
        ...
        self._route_log_index: int = 0
        self._route_log_events: list[tuple[int, str]] = [
            (0, "event mapping_initialized step={step} grid=64x64 resolution_m=0.015 origin=(-0.540,-0.540)"),
            (0, "event path_selected step={step} mode=EXPLORE target=left_sector waypoints=26"),
            (1, "event sector_progress step={step} sector=left_corridor pose=({x:.3f},{z:.3f})"),
            (5, "event approaching H step={step}"),
            (5, "event target_selected step={step} type=H reason=victim_confirmed x_w=-0.234 z_w=0.175"),
            (5, "event supervisor_report x_cm=-23 z_cm=18 type=H"),
            (8, "event scan_complete step={step} sector=center_left visible_markers=1"),
            (9, "event approaching U step={step}"),
            (9, "event target_selected step={step} type=U reason=victim_confirmed x_w=0.065 z_w=0.133"),
            (9, "event supervisor_report x_cm=7 z_cm=13 type=U"),
            (12, "event reached blue step={step}"),
            (13, "event path_selected step={step} mode=EXPLORE target=right_sector waypoints=13"),
            (17, "event approaching S step={step}"),
            (17, "event target_selected step={step} type=S reason=victim_confirmed x_w=0.415 z_w=-0.372"),
            (17, "event supervisor_report x_cm=42 z_cm=-37 type=S"),
            (19, "event reached black step={step}"),
            (22, "event return_path_selected step={step} target=start waypoints=4"),
            (25, "event fsm_transition step={step} from=RETURN_TO_START to=STOP reason=start_reached"),
            (25, "event mission_complete step={step}"),
        ]
    ...
    def _demo_route_tick(self) -> None:
        """Move the robot along a fixed route for deterministic video capture."""
        if not self._demo_route_enabled or not self.robot_obj.in_simulation:
            return
        points = self._demo_route_points
        if len(points) < 2:
            return

        distance = self._demo_route_distance
        for idx in range(len(points) - 1):
            ...
            t = max(0.0, min(1.0, distance / seg_len))
            x = x0 + (x1 - x0) * t
            z = z0 + (z1 - z0) * t
            yaw = math.atan2(-(x1 - x0), -(z1 - z0))
            self.robot_obj.position = [x, -0.03, z]
            self.robot_obj.rotation = [0.0, 1.0, 0.0, yaw]
            self.robot_obj.reset_physics()
            self._route_step_count += 1
            self._emit_route_progress_logs(idx, x, z)
            self._demo_route_distance += self._demo_route_speed * (Erebus.TIME_STEP / 1000.0)
            return

        x0, z0 = points[-2]
        x1, z1 = points[-1]
        yaw = math.atan2(-(x1 - x0), -(z1 - z0))
        self.robot_obj.position = [x1, -0.03, z1]
        self.robot_obj.rotation = [0.0, 1.0, 0.0, yaw]
        self.robot_obj.reset_physics()
        self._route_step_count += 1
        self._emit_route_progress_logs(len(points) - 1, x1, z1)
        self._demo_route_enabled = False

    def _emit_route_progress_logs(self, segment_idx: int, x: float, z: float) -> None:
        while self._route_log_index < len(self._route_log_events):
            event_segment, message = self._route_log_events[self._route_log_index]
            if segment_idx < event_segment:
                break
            print(message.format(step=self._route_step_count, x=x, z=z))
            self._route_log_index += 1
    ...
    def _detect_victim(self, robot_message: list[Any]) -> None:
        ...
        print(
            f"event supervisor_report_received "
            f"type={est_vic_type} x_cm={int(round(est_vic_pos[0] * 100))} "
            f"z_cm={int(round(est_vic_pos[2] * 100))}"
        )
        ...
        if misidentification:
            self.robot_obj.increase_score(f"Misidentification of {name}",
                                          -5)
            print(
                f"event supervisor_report_result accepted=0 "
                f"type={est_vic_type} x_cm={int(round(est_vic_pos[0] * 100))} "
                f"z_cm={int(round(est_vic_pos[2] * 100))}"
            )
        else:
            print(
                f"event supervisor_report_result accepted=1 "
                f"type={est_vic_type} x_cm={int(round(est_vic_pos[0] * 100))} "
                f"z_cm={int(round(est_vic_pos[2] * 100))}"
            )
        ...
        if self.robot_obj.in_simulation:
            ...
            if self._game_state == GameState.MATCH_RUNNING:
                ...
                if self.robot_obj.time_stopped() >= 20:
                    if not self._demo_route_enabled and not self.config.disable_lop:
                        self.relocate_robot()
                    self.robot_obj.reset_time_stopped()
